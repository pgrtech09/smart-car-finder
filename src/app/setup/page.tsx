      ...
      <div className="p-4 pb-6 max-w-lg mx-auto space-y-5">
        ...
        <div className="flex rounded-xl bg-surface-800 p-1 gap-1">
          {(["cars", "profile", "about"] as const).map((tab) => (
            ...
            >
              {tab === "cars" ? "Cars" : tab === "profile" ? "Profile" : "About"}
            </button>
          ))}
        </div>
        ...
        {activeTab === "cars" && (
          <div className="space-y-3">
            {!showForm && (
              <Button variant="primary" icon={<Plus size={15} />} onClick={() => handleOpenForm()} className="w-full justify-center">
                Add New Car
              </Button>
            )}
            ...
            {showForm && (
              <Card className="space-y-4 border border-brand-500/20">
                <CardHeader>
                  ...
                  <button onClick={() => setShowForm(false)} className="text-slate-500 hover:text-white text-xs">Cancel</button>
                </CardHeader>
                <div className="space-y-3">
                  <div>
                    <label className="text-xs text-slate-400 mb-1.5 block">Car Nickname</label>
                    <input type="text" placeholder="e.g. My Honda" value={formData.car_name}
                      onChange={(e) => updateField("car_name", e.target.value)}
                      className="w-full bg-surface-800 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-slate-500" />
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 mb-1.5 block">Car Model</label>
                    <input type="text" placeholder="e.g. Honda City 2022" value={formData.car_model}
                      onChange={(e) => updateField("car_model", e.target.value)}
                      className="w-full bg-surface-800 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-slate-500" />
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 mb-1.5 block">Bluetooth Device Name</label>
                    <div className="relative">
                      <Bluetooth size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
                      <input type="text" placeholder="e.g. Honda City Audio" value={formData.bluetooth_device_name}
                        onChange={(e) => updateField("bluetooth_device_name", e.target.value)}
                        className="w-full bg-surface-800 border border-white/10 rounded-xl pl-9 pr-3 py-2.5 text-sm text-white placeholder-slate-500" />
                    </div>
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 mb-1.5 block">License Plate</label>
                    <input type="text" placeholder="e.g. MH 01 AB 1234" value={formData.license_plate}
                      onChange={(e) => updateField("license_plate", e.target.value.toUpperCase())}
                      className="w-full bg-surface-800 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white placeholder-slate-500 font-mono" />
                  </div>
                  <div>
                    <label className="text-xs text-slate-400 mb-2 block">Car Color</label>
                    <div className="flex gap-2 flex-wrap">
                      {CAR_COLORS.map((color) => (
                        <button key={color} onClick={() => updateField("color", color)}
                          className={`w-7 h-7 rounded-full border-2 transition-all ${formData.color === color ? "border-white scale-110" : "border-transparent"}`}
                          style={{ backgroundColor: color }} />
                      ))}
                    </div>
                  </div>
                </div>
                <Button variant="primary" icon={saved ? <Check size={15} /> : <Save size={15} />}
                  ...
                  <Card key={car.id} className="border border-white/5">
                    <div className="flex items-center gap-3">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
                        ...
                        <Car size={20} style={{ color: car.color || "#4c6ef5" }} />
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        {car.license_plate && <p className="text-xs text-slate-500 font-mono">{car.license_plate}</p>}
                        {car.bluetooth_device_name && (
                          <p className="text-xs text-brand-400/70 flex items-center gap-1 mt-0.5">
                            <Bluetooth size={10} />{car.bluetooth_device_name}
                          </p>
                        )}
                      </div>
                      <div className="flex gap-1">
                        <button onClick={() => handleOpenForm(car)}
          ...
          <div className="space-y-3">
            <Card className="border border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-white truncate">{user?.user_metadata?.full_name ?? "Driver"}</p>
                  <p className="text-xs text-slate-400 truncate">{user?.email}</p>
                </div>
              </div>
            </Card>
            {[
              { icon: <Bell size={16} className="text-yellow-400" />, bg: "bg-yellow-500/10", label: "Notifications", sub: "Parking reminders" },
              { icon: <Shield size={16} className="text-green-400" />, bg: "bg-green-500/10", label: "Privacy", sub: "Manage your data" },
            ].map((item) => (
              <Card key={item.label} className="border border-white/5 cursor-pointer">
                <div className="flex items-center gap-3">
            ...
            <Button variant="danger" icon={<LogOut size={15} />} onClick={signOut} className="w-full justify-center">
              Sign Out
            </Button>
          </div>
        )}
          ...
          <div className="space-y-3">
            <Card className="border border-white/5">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                Smart Car Finder automatically saves your parking location when your phone disconnects from your car Bluetooth.
              </p>
            </Card>
            <Card className="border border-white/5 space-y-2">
              <p className="text-sm font-semibold text-white flex items-center gap-1.5">
                <Info size={14} className="text-brand-400" /> How It Works
              </p>
              <ul className="space-y-1.5">
                {[
                  "Pair your car Bluetooth in Setup",
                  "Connect your phone to car audio",
                  "App detects disconnect and saves GPS location",
                  "Open Map screen to navigate back to your car",
                ].map((item, i) => (
            ...
            <p className="text-center text-xs text-slate-600 py-2">Built with Next.js and Supabase</p>
          </div>
        )}
      </div>
    </AppShell>
